...
(function($) {
    ...
    function RenderData(data, element){
        for(var i = 0 ; i< data.length ; i++){
            ...
            console.log(options.multiSelect && data[i].dataAttrs[0].data)
            if(!element.is("li")){
                element.append('<li id="TreeElement'+globalTreeIdCounter+'"'+dataAttrs+'>'+((options.multiSelect && data[i].dataAttrs[0].data)?'<i class="fa fa-square-o select-box" aria-hidden="true"></i>':'')+'<a href="'+((typeof data[i].href != "undefined" && data[i].href!=null)?data[i].href:'#')+'">'+data[i].title+'</a></li>');
                ...
                element.find("ul").append('<li id="TreeElement'+globalTreeIdCounter+'"'+dataAttrs+'>'+((options.multiSelect && !data[i].dataAttrs[0].data)?'<i class="fa fa-square-o select-box" aria-hidden="true"></i>':'')+'<a href="'+((typeof data[i].href != "undefined" && data[i].href!=null)?data[i].href:'#')+'">'+data[i].title+'</a></li>');
                if(data[i].data != null && typeof data[i].data !="undefined"){
                    $("#TreeElement"+globalTreeIdCounter).append("<ul style='display:none'></ul>");
                    $("#TreeElement"+globalTreeIdCounter).find("a").first().prepend('<span class="arrow">'+options.closedArrow+'</span>');
                    RenderData(data[i].data, $("#TreeElement"+globalTreeIdCounter).find("ul").first());
    ...
    function click_expand(e) {
        e.stopPropagation();
        if (!lastChecked) lastChecked = this;

        if (e.shiftKey) {
            boxes = $(this).parents().first().find('li');
            ...
            boxes.slice(Math.min(start, end), Math.max(start, end) + 1).each(function() {
                ...
                if ($(this).find("i").first().hasClass("fa-square-o")) {
                    $(this).find("i").first().removeClass("fa-square-o");
                    $(this).find("i").first().addClass("fa-check-square-o");          
                }
                // else {
                //     $(this).find("i").first().addClass("fa-square-o");
                //     $(this).find("i").first().removeClass("fa-check-square-o");  
                // }
        ...
        if (e.currentTarget.getAttribute("dataselectable") == "false") {
            
            e.stopPropagation();
            $(this).find(".arrow").empty();
            var expanded;
            if($(this).find("ul").first().is(":visible")){
                expanded = false;
                $(this).find(".arrow").prepend(options.closedArrow);
                $(this).find("ul").first().hide();
            }else{
                expanded = true;
                $(this).find(".arrow").prepend(options.openedArrow);
                $(this).find("ul").first().show();
            }

            options.expandHandler($(this), e, expanded);
        }
            ...
            e.stopPropagation();
            var checked;
            select_box = $(this).find(".select-box").first()
            console.log(select_box)
            if(select_box.hasClass("fa-square-o")){
                //will select
                checked=true;
                select_box.removeClass("fa-square-o");
                select_box.addClass("fa-check-square-o");
                if(options.selectChildren){
                    $(this).find(".select-box").removeClass("fa-square-o");
                    $(this).find(".select-box").addClass("fa-check-square-o");
                    options.clickHandler($(this), e);
                }
            }else{
                //will unselect
                checked=false;
                select_box.addClass("fa-square-o");
                select_box.removeClass("fa-check-square-o");
                if(options.selectChildren){
                    $(this).find(".select-box").addClass("fa-square-o");
                    $(this).find(".select-box").removeClass("fa-check-square-o");
    ...
    $(options.element).on("click","li",click_expand);
    //$(options.element).on("click",".arrow",click_expand);
    ...
    $("#form").submit( function(eventObj) {
        ...
        plot_types = $("#example").GetSelected().toString();
        console.log(document.getElementById("plot_types"));
        document.getElementById("plot_types").value = plot_types;
        return true;
    });
